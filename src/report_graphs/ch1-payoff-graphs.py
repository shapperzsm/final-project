...
y_domain = [0, 1]
row_payoff = [
    [(A * sigma_c)[i].subs({y: value}) for value in y_domain] for i in range(2)
]
...
x_domain = [0, 1]
col_payoff = [
    [(sigma_r * B)[i].subs({x: value}) for value in x_domain] for i in range(2)
]
plt.plot(x_domain, col_payoff[0], label="$(\sigma_rB)_1$")
plt.plot(y_domain, row_payoff[1], label="$(\sigma_rB)_2$")
plt.xlabel("$\sigma_r=(x, 1-x)$")
plt.title(
    "Col. player's payoffs, when the row player's strategy is $\sigma_r=(x, 1-x)$"
)
plt.legend()
