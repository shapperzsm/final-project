...
threshold_file = p / "main_no_degeneracy_full-new.csv"
...
could_be_degenerate_tournaments = list(pd.DataFrame(connect_dbms_to_db.execute(degenerate_tournament_set_collection).fetchall()).drop_duplicates()[0])
...
non_degenerate_tournaments = [list(range(maximum_player_set)).remove(i) for i in could_be_degenerate_tournaments]

for each_set in non_degenerate_tournaments:
