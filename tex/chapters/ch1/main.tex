...
\ldots, s_{n}\}\), i.e. the \(i\)th player's strategy has been omitted. The set,
\(S_{-i}\), is defined similarly. Looking at the row player's matrix of a
...
\begin{center}
    ...
    for A Prisoner's Dilemma, with a payoff value of 1 for each player.}
...
\begin{figure}\label{fig:PD_game_tree}
    \begin{tikzpicture}
        \tikzstyle{red hollow node}[circle, draw=red, inner sep=2]
        \tikzstyle{blue hollow node} [circle, draw=blue, inner sep=2]
        \tikzstyle{black hollow node} [circle, draw=black, inner sep=2]
        \node(0)[red hollow node]{player 1}
            [grow=east]
            child{node(1)[blue hollow node]{player 2} 
                child{node[black hollow node, label=right:{$(3,3)$}]{} edge from parent[blue]{co-op}}
                child{node[black hollow node, label=right:{$(0,5)$}]{} edge from parent[blue]{defect}}
                edge from parent[red]{co-op}
            }
            child{node(2)[blue hollow node]{player 2} 
                child{node[black hollow node, label=right:{$(5, 0)$}]{} edge from parent[blue]{co-op}}
                child{node[black hollow node, label=right:{$(1, 1)$}]{} edge from parent[blue]{defect}}
                edge from parent[red]{defect}
            };
...
\]
where the discount factor, \(\delta\), can be thought of as the probability that
the game continues. That is, the probability that another stage game will be
played. Using this, the \textit{average payoffs}, per stage game, can be defined by:
\[
...
\begin{figure}\label{fig:Feasible_Payoff_Plot}
    \begin{tikzpicture}
        ...
        \draw[black, dashed] (1,1) -- (0,5) -- (3,3) -- (5,0) cycle;
...
from~\cite{friedman1971non} as well as from~\cite{Webb2007}.
...
\begin{enumerate}
    \item The mixed strategy sets, \(\Sigma_{i}\) are compact and convex for all
    \(i\in N\). 
    ...
    \in \mathbb{R}\), the set \(\{x \in C : f(x) \ge a\}\) is convex.} functions
    of \(sigma_{i}\) for all \(i\in N\).
...
\begin{theorem}\label{thm:Folk}
    ...
    game, then there exists a strategy, \(\sigma^{*}\) which satisfies the
    following two conditions:
    \begin{itemize}
        \item \(\sigma^{*}\) is an element of the set of strategy vectors which
        obtain Pareto optimal payoffs; and
        \item \( \frac{U_{i}(\sigma_{-i}^{*},t_{i})-U_{i}(\sigma^{ne})}
        {U_{i}(\sigma^{*})-U_{i}(\sigma^{ne})} = 
        \frac{U_{j}(\sigma_{-j}^{*},t_{j})-U_{j}(\sigma^{ne})}
        {U_{j}(\sigma^{*})-U_{j}(\sigma^{ne})}\)
    \end{itemize}
    where \(\sigma^{ne}\) is the strategy consisting of repeated play of the
    stage Nash equilibrium, \(t_{i}\) is a strategy for player \(i\) which
    maximises a single stage game payoff against \(\sigma_{-i}^{*}\) and \(i, j
    \in \{1, 2, \ldots, N\}\).
    In addition, if the inequality:
    \[
        \frac{1-\delta_{i}}{\delta_{i}} < \frac{U_{i}(\sigma^{*})-U_{i}(\sigma^{ne})}{\phi_{i}(\sigma_{-i}^{*})-U_{i}(\sigma^{ne})}
    \]
    holds for all \(i \in \{1, 2, \ldots, N\}\), then \(\sigma^{*}\) is a Nash
    equilibrium. Here, \(\delta_{i}\) is the discount factor and function \(\phi\)
\end{theorem}

\begin{proof}
    Firstly, the existence of such a strategy, \(\sigma^{*}\) needs to be
    proved. This is done as follows.
