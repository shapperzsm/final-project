...
created~\footnote{Please see Appendix~\ref{} for the code used to obtain these files.} containing the minimum, mean, median and maximum probabilities for each
...
From Figures~\ref{fig:mean_median_p_thresh} and~\ref{fig:min_max_p_thresh},
observe that all have a significant amount of thresholds at 0.001 and 0.999,
giving distinct lines at these points. This implies that, for a certain number
of tournaments, it is either more beneficial to always defect (when the
threshold is 0.001) or best to never defect (indicated by the threshold at
0.001). On the other hand, the \(p\)-thresholds seem to
be quite erratic, with the range of these thresholds depending on which
statistic is chosen. Specifically, for the minimum and maximum \(p\)-threshold
the majority of thresholds lie between 0 and 0.4 whereas, for the mean and
median, a significant amount of the thresholds lie between 0 and 0.6. Comparing
Figures~\ref{fig:mean_median_p_thresh} and~\ref{fig:min_max_p_thresh} to
Figures~\ref{fig:non_degen_min_max_p_thresh}
and~\ref{fig:non_degen_mean_median_p_thresh}, the corresponding \(p\)-thresholds
for only the non-degenerate games, it can be seen that the plots are
approximately the same. Degeneracy only seems to affect the higher tournament
player sets, that is, those which have a greater number of players. However,
this could be due to only a small amount of the tournaments leading to
degenerate games. Effects of degeneracy will be investigated in further detail
in Section~\ref{subsec:Effects_of_Degeneracy}.
...
Now, the different characteristics of a tournament will be discussed in relation
with its effect on the \(p\)-threshold.
